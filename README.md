...
로그인 화면의 **테스트 모드로 둘러보기**를 누르거나 주소 뒤에 `?demo=1`을 붙이면 가상 가족 데이터로 앱 전체를 확인할 수 있습니다.

```text
http://localhost:4173/?demo=1
https://wys1110.github.io/family/?demo=1
```

테스트 모드에서는 Supabase 클라이언트를 생성하지 않으므로 실제 가족 DB를 읽거나 수정하지 않습니다. 일정·성장 기록·아기 프로필은 `family-demo-*` 전용 로컬 저장 키에만 보관됩니다. 상단의 **테스트 종료**를 누르면 실제 로그인 화면으로 돌아가며, 계정 팝업의 **가상 데이터 초기화**로 기본 샘플을 다시 만들 수 있습니다.
