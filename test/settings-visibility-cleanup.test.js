...
test("설정 화면에서 사용하지 않는 카드와 알림 카드를 숨긴다", () => {
  ...
  expect(settingsStyle).toMatch(
    /#settingsView\s*>\s*#dailyBriefingSettings[\s\S]*?display:\s*none\s*!important;/,
  );
  expect(settingsStyle).toMatch(
...
test("숨긴 설정 모듈의 생성 코드는 미래 재노출을 위해 유지한다", () => {
  ...
  expect(dailyBriefingSource).toContain('card.id = "dailyBriefingSettings"');
